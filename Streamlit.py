...
def main():
    ...
    st.title("Криптопрогнозирование с помощью Arima")
    st.subheader("Проект Перебаскина Сергея (ТРП-2-19)")
    st.text("Информация собрана с 'coinbase' ")


    st.sidebar.write("Выберите валюту и период")
    coins = st.sidebar.selectbox("Выбор валюты", (tup))
    ...
    st.write("Подготовка к вычислениям.")
    st.write("Помолимся и понадеемся на хороший результат")
    period = st.slider("Выбор периода", 1, 5, 1)
    if st.button("Начало предсказания"):
